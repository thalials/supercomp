...
int M(int N, vector<int> &P, vector<int> &V, int C, int i, vector<bool> &used,
        vector<bool> &best, int &num_leaf, int &num_copy, 
        vector<bool> &aux, vector<int> &bound, vector<int> &valoresBounds) {
...
int main()  {
    ...
    int value = M(n, peso, valor, C, 0, used, best, num_leaf, num_copy, aux, bound, valoresBound);
